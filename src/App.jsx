...
import Tooltip from "@mui/material/Tooltip";
import { useState } from "react";
import ColorModal from "./components/color-modal/index"; // Importing the new modal

const App = () => {
  const count = useSelector((store) => store.counter.count);
  const bgColor = useSelector((store) => store.color.bgColor); // Getting the background color from Redux
  const dispatch = useDispatch();
